...
function createTab() {
  ...
  document.querySelectorAll(".btn").forEach((x) => {
    ...
    x.addEventListener("touchstart", handleTouchStart); // Handle long press start on mobile
    x.addEventListener("touchend", handleTouchEnd); // Handle long press end on mobile
  });
}
function clic(e) {  
  ...
  } else if (e.button == 2 || e.touches != null) {
...
function handleTouchEnd(e) {
  ...
  clearTimeout(longPressTimer); // If the user releases before long press duration, cancel it
}
